...
export function Movimentacoes() {
  ...
  const materialSelecionado = materiais.find((m) => m.id === materialId) || null;

  const materiaisFiltrados =
    buscaMaterial.trim().length === 0
      ? materiais
  ...
  function selecionarMaterial(m: Material) {
    setMaterialId(m.id);
    setBuscaMaterial('');
      ...
      <div className="grid grid-cols-2 gap-3.5 max-w-lg">
        <div className="col-span-2 relative">
          ...
          <input
            ...
            value={listaAberta ? buscaMaterial : materialSelecionado ? `${materialSelecionado.nome} — saldo atual: ${materialSelecionado.estoqueAtual} ${materialSelecionado.unidade}` : ''}
            onFocus={() => {
              setListaAberta(true);
              setBuscaMaterial('');
            }}
          ...
          />
          {listaAberta && (
            <div className="absolute z-20 mt-1 w-full max-h-64 overflow-y-auto bg-white border border-linha rounded-lg shadow-lg">
              ...
              {materiaisFiltrados.map((m) => (
                <div
                  key={m.id}
                  onMouseDown={() => selecionarMaterial(m)}
                  className={`px-3 py-2.5 text-[12.5px] cursor-pointer hover:bg-fundo ${
                    m.id === materialId ? 'bg-azulClaro text-azul font-semibold' : 'text-texto'
                  }`}
                >
                  {m.nome} <span className="text-textoSuave">— saldo atual: {m.estoqueAtual} {m.unidade}</span>
                </div>
              ))}
            </div>
          )}
